...
CC_465 = load_workbook("Expense_465W_CC.XLSX")
...
# Merge 파일 Title 지정
ws.append(['Cost Center', 'Cost Element', 'Val/COArea Crcy', 'Partner object', 'Name', 'Org Cost Center', 'Partner object type'])
...
Final.delete_cols(8) # delete column 삭제
...
special = wb.create_sheet('62990000')
special.append(['Cost Center', 'Cost Element', 'Val/COArea Crcy', 'Partner object', 'Name', 'Org Cost Center', 'Partner object type'])
for cell in Final["B"]:
    list_62990000 = []
    if cell.value == '62990000':
        row = Final[cell.coordinate[1:]]
        for cell in row:
            list_62990000.append(cell.value)
        special.append(list_62990000)
        
##### 셀 너비 조정 및 단위 변경

for cell in special["C"]:
    cell.number_format = openpyxl.styles.numbers.builtin_format_code(3)

for column_cells in special.columns:
    length = max(len(str(cell.value))*1.1 for cell in column_cells)
    special.column_dimensions[column_cells[0].column_letter].width = length
