...
# for row in ws[:ws.max_row]:
#     Final_data = [cell.value for cell in row]
#     print(Final_data)
#     final_ws.append(Final_data)
final_ws = wb.copy_worksheet(ws)
final_ws.title = 'Final'

######## Final 값들만 남기고 Delete되는 행 데이터 지우기
for row in final_ws[2:final_ws.max_row]: # 전체 행
    for cell in row:
        if final_ws["H" + cell.coordinate[1:]].value == 'Delete': # H열이 Delete면
            cell.value = "" # 데이터 지우기
...
#     path = r"C:\Users\z004ncux\Desktop\work\Python\Automation_with_Python\Automation_with_Python\Expense_Actual_Update\Expense_Actual_Update_Download - 복사본.xlsm"
#     #매크로 파일 설정
